...
/*   Updated: 2025/03/14 19:35:32 by abreuil          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int							check_consecutive_ops(char *curr_token,
								char *next_token);
int							handle_operator(t_shell *shell, int token_count,
								int i);
...
t_simple_cmds				*handle_redirection(t_simple_cmds *cmd,
								t_shell *shell);
...
int							process_token(t_shell *shell, t_simple_cmds *cmd,
								int *i);
...
int							get_redirection_tokens(t_shell *shell, char **token,
								char **target);
t_redir						*create_redirection(char *token, char *target);
t_simple_cmds				*prepare_simple_cmd(t_shell *shell,
								int *word_count);
